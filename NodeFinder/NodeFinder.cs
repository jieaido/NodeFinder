...
namespace NodeFinder
{
    public class PathFinder
    {
        ...
        public bool SerachGoal(Postion startPostion, Postion goalpostion,out string Msg)
        {
            ...
            {
                Msg = "no found startNode or GoalNode";
                return false;
            }
            _startNode.StartG = 0;
            ...
            while (OpenNodes.Count>0)
            {
                ...
                if (calcNode.Position!=_goalNode.Position)
                {
                        ...
                        if (CloseNodes.Exists(s => s.Position == CalcNodes[i].Position))
                            {
                                ...
                                var p = CloseNodes.Find(s => s.Position == CalcNodes[i].Position);
                                if (CalcNodes[i].TotalF < p.TotalF)
                                {
                                    p.ParentNode = calcNode;
                                    CloseNodes.Remove(p);
                ...
                {
                    List<PathNode> pps=new List<PathNode>();
                    var p = calcNode.ParentNode;
                    while (p!=null)
                    {
                        pps.Add(p);
                        p = p.ParentNode;
                    }
                    foreach (var pi in pps)
                    {
                        Msg += pi.Position.ToString();
                    }
                    return true;
                }
        ...
        private void FindGroundNodes(PathNode pn,out List<PathNode> pns )
        {
            ...
            var p1=  Nodes.Find(p=>p.Position.X==pn.Position.X&&p.Position.Y==pn.Position.Y+1);
            var p2=  Nodes.Find(p=>p.Position.X==pn.Position.X&&p.Position.Y==pn.Position.Y-1);
            var p3=  Nodes.Find(p=>p.Position.X==pn.Position.X+1&&p.Position.Y==pn.Position.Y);
            var p4=  Nodes.Find(p=>p.Position.X==pn.Position.X-1&&p.Position.Y==pn.Position.Y);
